    ...
    <div className="mx-auto max-w-[1200px] px-6 py-12">
      <div className="mb-8">
        <h1 className="text-2xl font-black uppercase tracking-wider text-foreground">
          {t("Welcome", "Bem-vindo")}, {user?.name?.split(" ")[0] || "Gamer"}
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">{t("Your account activity overview.", "Visao geral da atividade da sua conta.")}</p>
      </div>

      {stats && (
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <StatsCard icon={ShoppingBag} label={t("ORDERS", "PEDIDOS")} value={String(stats.orders)} />
          <StatsCard icon={DollarSign} label={t("SPENT", "GASTO")} value={`$${stats.spent.toFixed(2)}`} />
          <StatsCard icon={Heart} label={t("FAVORITES", "FAVORITOS")} value={String(stats.favorites)} />
          <StatsCard icon={MessageSquare} label={t("REVIEWS", "AVALIACOES")} value={String(stats.reviews)} />
        </div>
      )}
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        {lastOrder && (
          <Card className="border-border bg-card">
            <CardHeader>
              <CardTitle className="text-sm font-black uppercase tracking-wider text-foreground">{t("Last Order", "Ultimo Pedido")}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <Button variant="outline" size="sm" asChild className="w-full border-border text-xs font-bold uppercase tracking-wider text-muted-foreground hover:border-primary hover:text-primary">
                <Link href="/track-order">{t("Track Order", "Rastrear Pedido")}</Link>
              </Button>
            </CardContent>
          </Card>
        )}
        ...
        <Card className="border-border bg-card">
          <CardHeader>
            <CardTitle className="text-sm font-black uppercase tracking-wider text-foreground">{t("Quick Links", "Links Rapidos")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            ...
            <Button variant="outline" asChild className="w-full justify-start gap-2 border-border text-sm text-muted-foreground hover:border-primary hover:text-primary">
              <Link href="/track-order">
                <Package className="h-4 w-4" /> {t("Track Orders", "Rastrear Pedidos")}
              </Link>
            </Button>
            <Button variant="outline" asChild className="w-full justify-start gap-2 border-border text-sm text-muted-foreground hover:border-primary hover:text-primary">
              <Link href="/support">
                <Headphones className="h-4 w-4" /> {t("Support", "Suporte")}
              </Link>
            </Button>
          </CardContent>
        </Card>
