  ...
  return (
    <div>
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        <h3 className="section-title">Vistos recentemente</h3>
        <span className="badge-muted">Seu historico</span>
      </div>
      <div className="flex gap-4 overflow-x-auto pb-2">
        {viewed.map(product => (
          <div key={product.id} className="min-w-[200px] product-card flex flex-col">
            ...
            <span className="price-tag text-xs">{formatFromBase(product.price)}</span>
          </div>
        ))}
      </div>
    </div>
