...
export default function WebhooksPage() {
  ...
  const [form, setForm] = useState<WebhookForm>({ event: '', url: '', enabled: true });
      ...
      <ul className="space-y-2">
        {webhooks.length === 0 ? (
          ...
          webhooks.map((webhook) => (
            <li key={webhook.id} className="border rounded p-2 bg-gray-50 flex justify-between items-center">
              <div>
                <div className="font-semibold">{webhook.event}</div>
                <div className="text-xs text-gray-600">URL: {webhook.url}</div>
                <div className="text-xs text-gray-400">{webhook.enabled ? 'Ativo' : 'Desativado'}</div>
              </div>
              {/* ...ações futuras... */}
            </li>
          ))
