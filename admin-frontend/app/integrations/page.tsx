...
export default function IntegrationsPage() {
  ...
  const [editForm, setEditForm] = useState<IntegrationForm>({ type: '', apiKey: '', enabled: false });
  ...
  const [form, setForm] = useState<IntegrationForm>({ type: '', apiKey: '', enabled: false });
  ...
  const startEdit = (integration: Integration) => {
    ...
    setEditForm({ type: integration.type, apiKey: integration.apiKey, enabled: integration.enabled });
  };
  ...
  const saveEdit = async (id: string) => {
    ...
    try {
      ...
      setIntegrations(list => list.map(i => i.id === id ? { ...i, ...editForm } : i));
      setEditId(null);
    } catch {
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white border rounded-xl shadow">
      ...
      <div className="mb-4 flex gap-2">
        <button onClick={exportCSV} className="bg-gray-600 text-white px-4 py-1 rounded font-semibold">Exportar CSV</button>
        <button onClick={exportExcel} className="bg-gray-800 text-white px-4 py-1 rounded font-semibold">Exportar Excel</button>
      </div>
      <ul className="space-y-2">
        {integrations.length === 0 ? (
          ...
          integrations.map((integration) => (
            <li key={integration.id} className="border rounded p-2 bg-gray-50 flex justify-between items-center">
              <div>
                {editId === integration.id ? (
                  <form onSubmit={e => { e.preventDefault(); saveEdit(integration.id); }} className="flex gap-2 items-center">
                    <select name="type" value={editForm.type} onChange={handleEditChange} className="border rounded px-2 py-1">
                      <option value="pagamento">Pagamento</option>
                      <option value="frete">Frete</option>
                      <option value="notificacao">Notificação</option>
                      <option value="crm">CRM</option>
                    </select>
                    <input type="text" name="apiKey" value={editForm.apiKey} onChange={handleEditChange} className="border rounded px-2 py-1" />
                    <label className="flex items-center gap-1">
                      <input type="checkbox" name="enabled" checked={editForm.enabled} onChange={handleEditChange} /> Ativa
                    </label>
                  ...
                  <>
                    ...
                    <div className="text-xs text-gray-600">API Key: {integration.apiKey}</div>
                    <div className="text-xs text-gray-400">{integration.enabled ? 'Ativa' : 'Desativada'}</div>
                  </>
                )}
              </div>
              <div className="flex gap-2">
                {editId !== integration.id && (
                  <button className="bg-blue-600 text-white px-2 py-1 rounded text-xs" onClick={() => startEdit(integration)}>Editar</button>
                )}
                <button className="bg-red-600 text-white px-2 py-1 rounded text-xs" onClick={() => removeIntegration(integration.id)}>Remover</button>
                <button className="bg-gray-600 text-white px-2 py-1 rounded text-xs" onClick={() => fetchLogs(integration.id)}>Logs</button>
                <button className="bg-green-600 text-white px-2 py-1 rounded text-xs" onClick={() => testConnection(integration.id)} disabled={testingId === integration.id}>Testar conexão</button>
                {testingId === integration.id && <span className="text-xs text-blue-600 ml-2">Testando...</span>}
                {testResult && showLogsId !== integration.id && testingId === null && <span className="text-xs text-gray-700 ml-2">{testResult}</span>}
              </div>
            </li>
          ))
        )}
      ...
      {showLogsId && (
        <div className="mt-8">
          ...
          <button className="mb-2 text-blue-600 underline text-xs" onClick={() => setShowLogsId(null)}>Fechar</button>
          {logs.length === 0 ? (
