...
export default function AdminsPage() {
  ...
  const exportCSV = () => {
    ...
    const rows = admins.map(a => [a.name, a.email, a.role, a.blocked ? 'Bloqueado' : 'Ativo']);
  ...
  const saveRoleEdit = async () => {
    ...
    try {
      ...
      setAdmins(admins =>
        admins.map(a => a.id === roleEdit.id ? { ...a, role: roleEdit.role } : a)
      );
      setRoleEdit(null);
    } catch {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white border rounded-xl shadow">
      ...
      <div className="mb-6 flex gap-2 flex-wrap">
        ...
        />
        <button onClick={inviteAdmin} className="bg-blue-600 text-white px-4 py-1 rounded font-semibold" disabled={inviting}>
        ...
        <textarea
          ...
          rows={1}
        />
        <button onClick={inviteBatchAdmins} className="bg-green-600 text-white px-4 py-1 rounded font-semibold" disabled={inviting || !inviteBatch.trim()}>
            ...
            <tr key={a.id}>
              ...
              <td className="p-2 border">
                {roleEdit && roleEdit.id === a.id ? (
                  <select value={roleEdit.role} onChange={e => setRoleEdit({ id: a.id, role: e.target.value })} className="border rounded px-2 py-1">
                    {roleOptions.map(r => <option key={r} value={r}>{r}</option>)}
                  </select>
                ) : (
                  <span>{a.role}</span>
                ...
                {(!roleEdit || roleEdit.id !== a.id) && (
                  <button className="ml-2 text-blue-600 underline text-xs" onClick={() => startRoleEdit(a.id, a.role)}>Editar</button>
                )}
                {roleEdit && roleEdit.id === a.id && (
                  <button className="ml-2 text-green-600 underline text-xs" onClick={saveRoleEdit} disabled={updating === a.id}>Salvar</button>
                )}
                {roleEdit && roleEdit.id === a.id && (
                  <button className="ml-2 text-red-600 underline text-xs" onClick={() => setRoleEdit(null)}>Cancelar</button>
                )}
              </td>
              <td className="p-2 border">{a.blocked ? 'Bloqueado' : 'Ativo'}</td>
              <td className="p-2 border">
                <button
                ...
                >
                  {updating === a.id ? 'Atualizando...' : a.blocked ? 'Desbloquear' : 'Bloquear'}
                </button>
                <button
      ...
      {showLogsId && (
        <div className="mt-8">
          <h2 className="text-lg font-semibold mb-2">Logs de acesso do admin</h2>
