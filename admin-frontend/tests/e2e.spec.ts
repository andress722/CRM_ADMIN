...
test('crm leads page loads', async ({ page }) => {
  ...
  const seen = await Promise.race([
    leadsHeader.waitFor({ state: 'visible', timeout: 3000 }).then(() => 'leads').catch(() => null),
    unauthenticated.waitFor({ state: 'visible', timeout: 3000 }).then(() => 'unauthenticated').catch(() => null),
    login.waitFor({ state: 'visible', timeout: 3000 }).then(() => 'login').catch(() => null),
  ]);
